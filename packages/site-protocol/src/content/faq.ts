...
export const faqs: FaqEntry[] = [
  ...
  },
  {
    ...
    answer:
      "Yes — that is Sign In With DFOS. The application mints a challenge, a key from the user's identity chain signs it, and the application verifies the signature against the chain resolved from any relay. Verifying the signed challenge is pure crypto — no DFOS server is in the loop for that check, and the verifier learns nothing it did not ask for. Scopes that return a credential add a second, explicitly server-side surface: the credential gates requests to the issuing platform's API, where the API host verifies each request itself. One challenge artifact travels by either of two couriers — a hosted web redirect for a user who is present, or a sign-request mailbox the user's own signer polls asynchronously — and a verifier cannot tell which one delivered it.",
    answerHtml:
      'Yes — that is <a href="/siwd">Sign In With DFOS</a>. The application mints a challenge, a key from the user\'s identity chain signs it, and the application verifies the signature against the chain resolved from any relay. Verifying the signed challenge is pure crypto — no DFOS server is in the loop for that check, and the verifier learns nothing it did not ask for. Scopes that return a credential add a second, explicitly server-side surface: the credential <a href="/api-auth">gates requests</a> to the issuing platform\'s API, where the API host verifies each request itself. One challenge artifact travels by either of two couriers — a hosted web redirect for a user who is present, or a <a href="/signing">sign-request mailbox</a> the user\'s own signer polls asynchronously — and a verifier cannot tell which one delivered it.',
  },
  {
    question: "Can an application call a DFOS API on the user's behalf?",
    answer:
      "Yes — with a credential issued at SIWD consent and a per-request proof of possession. The credential names the grant (issuer, audience, scope, expiry) and is useless as a bearer token: every request carries a short-lived request proof signed by the application's own key, binding the exact method, host, path, and body. The API host verifies both on every call, checks revocation, and refuses public audiences — the resource form is api:<host>, so a self-hosted deployment gets the same gating for free.",
    answerHtml:
      'Yes — with a credential issued at <a href="/siwd">Sign In With DFOS</a> consent and a per-request proof of possession. The credential names the grant (issuer, audience, scope, expiry) and is useless as a bearer token: every request carries a short-lived <a href="/api-auth">request proof</a> signed by the application\'s own key, binding the exact method, host, path, and body. The API host verifies both on every call, checks revocation, and refuses public audiences — the resource form is <code>api:&lt;host&gt;</code>, so a self-hosted deployment gets the same gating for free.',
  },
  {
  ...
  },
  {
    ...
    answer:
      '@metalabel/dfos-client, the high-level read client. The protocol library owns the crypto truth; the client owns the four things it refuses to do — fetch, resolve, verify-orchestration, and cache — across an untrusted set of relays. It holds no keys and never writes. Trust comes back as data rather than exceptions: every resolution reports what it could and could not prove, and tip freshness is never claimed as proven. Its /siwd subpath carries the relying-party login kit for Sign In With DFOS. Its /api-auth subpath signs and verifies the per-request proofs for credential-gated APIs.',
    answerHtml:
      '<a href="https://www.npmjs.com/package/@metalabel/dfos-client">@metalabel/dfos-client</a>, the high-level read client. The protocol library owns the crypto truth; the client owns the four things it refuses to do — fetch, resolve, verify-orchestration, and cache — across an untrusted set of relays. It holds no keys and never writes. Trust comes back as data rather than exceptions: every resolution reports what it could and could not prove, and tip freshness is never claimed as proven. Its <code>/siwd</code> subpath carries the relying-party login kit for <a href="/siwd">Sign In With DFOS</a>. Its <a href="/api-auth"><code>/api-auth</code></a> subpath signs and verifies the per-request proofs for credential-gated APIs.',
  },
  {
