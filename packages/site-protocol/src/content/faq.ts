...
export const faqs: FaqEntry[] = [
  ...
  },
  {
    ...
    answer:
      'AT Protocol and DFOS Protocol share some goals — self-sovereign identity, signed data, content-addressed storage — but differ structurally. AT Protocol is public-by-default: your data repository is a public document, posts are visible to the network, and federation relays ingest content openly. The DFOS Protocol inverts this topology. Content is private — it lives in member-governed spaces, visible only to participants. The cryptographic proof layer is the only public surface: signed chains that anyone can verify without seeing the content itself. This is a foundational architectural choice, not a privacy setting. Beyond topology, AT Protocol is a full social networking protocol (federation, data repositories, application schemas, social graph). The DFOS Protocol is narrower by design — it specifies only the cryptographic primitives: signed chains, CID derivation, DID resolution, and verification rules, and is agnostic to transport, federation, and application semantics.',
    answerHtml:
