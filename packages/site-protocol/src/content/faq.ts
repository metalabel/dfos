...
export const faqs: FaqEntry[] = [
  ...
  },
  {
    ...
    answer:
      'Relays are verifying HTTP endpoints that store and serve chains: every relay independently re-verifies every operation, authorship is verifiable without trusting any server, and topology emerges from per-peer configuration rather than roles or hierarchy. The Relay specification carries the read and write contracts, the routes and their shapes, the capability-gated profiles, the peering behaviors, and the DID resolver.',
    answerHtml:
      'Relays are verifying HTTP endpoints that store and serve chains: every relay independently re-verifies every operation, authorship is verifiable without trusting any server, and topology emerges from per-peer configuration rather than roles or hierarchy. The <a href="/relay">Relay specification</a> carries the read and write contracts, the routes and their shapes, the capability-gated profiles, the peering behaviors, and the DID resolver.',
  },
  {
  ...
  },
  {
    ...
    answer:
      "Yes — that is Sign In With DFOS: the application mints a challenge, a key from the user's identity chain signs it, and the application verifies the signature against the chain resolved from any relay — pure crypto for that check, no DFOS server in the loop. Scopes that return a credential add a second, explicitly server-side surface: the credential gates requests to the issuing platform's API. The Integrations specification defines the challenge, the two couriers, the verification rules, and the gated-request side.",
    answerHtml:
      'Yes — that is <a href="/integrations#sign-in">Sign In With DFOS</a>: the application mints a challenge, a key from the user\'s identity chain signs it, and the application verifies the signature against the chain resolved from any relay — pure crypto for that check, no DFOS server in the loop. Scopes that return a credential add a second, explicitly server-side surface: the credential gates requests to the issuing platform\'s API. The <a href="/integrations">Integrations specification</a> defines the challenge, the two couriers, the verification rules, and the gated-request side.',
  },
  {
