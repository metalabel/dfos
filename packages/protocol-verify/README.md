...
Cross-language verification of the [DFOS protocol](https://protocol.dfos.com/spec). Six independent implementations re-derive CIDs, verify Ed25519 signatures, build merkle trees, and check credential structures from the same reference fixtures — proving the protocol specification is unambiguous across languages.
...
9. **Merkle tree** — binary tree construction from sorted IDs + inclusion proof verification
10. **Beacon signatures** — merkle root announcements with countersignatures
11. **VC-JWT credentials** — W3C verifiable credentials (DFOSContentWrite, DFOSContentRead)
12. **Number encoding determinism** — integers MUST encode as CBOR integers, not floats
