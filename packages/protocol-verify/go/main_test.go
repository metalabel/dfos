...
func TestJWSDeleteRestoreVerification(t *testing.T) {
	...
	if deletePayload["type"] != "delete" || deletePayload["previousOperationCID"] != "bafyreibfuh63uv33i2i5eooe3boit2ruyjehubsryemuuz6mrtlej26rei" {
		t.Fatalf("wrong delete payload: %+v", deletePayload)
	}
	deletePayload["version"] = int64(1)
	if cidToBase32(makeCIDBytes(dagCborEncodeJSON(deletePayload))) != deleteHeader["cid"] || deleteHeader["cid"] != "bafyreicl3a2t6vhz5vgvs5ojdw5wcwgoz3taxqqwexpbpltm2gh3q42zyi" {
	...
	if cidToBase32(makeCIDBytes(dagCborEncodeJSON(restorePayload))) != restoreHeader["cid"] || restoreHeader["cid"] != "bafyreieyavue6vxzt63ulkqpwetfwqvfzdkeq6t3q3gwrjnqghmijrgyba" {
