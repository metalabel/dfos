...
func TestSigningDisabledRoutes501(t *testing.T) {
	...
	assertSigning501(t, "POST /signing/v1/requests", signingPost(t, base+"/signing/v1/requests", map[string]string{"request": "invalid"}, ""))
	resp, err := http.Get(base + "/signing/v1/requests")
	if err != nil {
		t.Fatal(err)
	}
	assertSigning501(t, "GET /signing/v1/requests", resp)
	assertSigning501(t, "POST /signing/v1/requests/{cid}/response", signingPost(t, base+"/signing/v1/requests/"+cid+"/response", map[string]string{"response": "invalid"}, ""))
	resp, err = http.Get(base + "/signing/v1/requests/" + cid + "/response")
	if err != nil {
		t.Fatal(err)
	}
	assertSigning501(t, "GET /signing/v1/requests/{cid}/response", resp)
	assertSigning501(t, "POST /signing/v1/requests/{cid}/decline", signingPost(t, base+"/signing/v1/requests/"+cid+"/decline", nil, ""))
}
