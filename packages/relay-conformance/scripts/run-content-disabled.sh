#!/usr/bin/env bash
#
# Run the content-disabled conformance variant against BOTH reference relays.
# Each twin advertises capabilities.content:false, returns 501 from every
# /content/v0 route, and leaves the proof/content planes enabled.
#
# Usage:
#   cd packages/relay-conformance && ./scripts/run-content-disabled.sh
...
# serve-content-disabled\.ts`, which reaped by script path and so reached
...
# makes every content-disabled test self-skip and the run reports success having
...
run_variant() {
  ...
  echo ""
  echo "=== $label content-disabled relay on :$port ==="
  curl -fsS "http://localhost:$port/.well-known/dfos-relay" |
    python3 -c 'import json, sys; assert json.load(sys.stdin)["capabilities"]["content"] is False, "expected disabled capability"'
  cd "$CONFORMANCE_DIR"
  go vet ./...
  RELAY_URL="http://localhost:$port" \
    go test -v -count=1 -timeout 90s -run 'TestContentDisabled' ./...
}

TS_PORT="$(free_port)"
assert_port_free "$TS_PORT"
echo "Starting TS content-disabled relay on :$TS_PORT..."
set -m
pnpm --filter @metalabel/dfos-web-relay exec tsx \
  "$SCRIPT_DIR/serve-content-disabled.ts" "$TS_PORT" &
TS_PID=$!
set +m
...
echo ""
echo "Building Go content-disabled relay..."
GO_BIN_DIR="$(mktemp -d)"
GO_BIN="$GO_BIN_DIR/content-disabled-serve"
( cd "$RELAY_GO_DIR" && go build -o "$GO_BIN" "$SCRIPT_DIR/serve-content-disabled.go" )

GO_PORT="$(free_port)"
assert_port_free "$GO_PORT"
echo "Starting Go content-disabled relay on :$GO_PORT..."
set -m
"$GO_BIN" "$GO_PORT" &
GO_PID=$!
set +m
...
echo ""
echo "✓ content-disabled conformance passed against both TS and Go reference relays"
