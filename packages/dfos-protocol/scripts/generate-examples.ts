 ...
 *
 * Usage: pnpm exec vitest run scripts/generate-examples.ts
 */
...
describe('generate examples', () => {
  it('writes fixture files to examples/', async () => {
    ...
    const { jwsToken: contentCreateJws, operationCID: contentCreateCID } =
      await signContentOperation({ operation: contentCreateOp, signer: signer2, kid: kid2 });
