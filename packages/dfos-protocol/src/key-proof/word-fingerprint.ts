/*
  ...
  alternating the PGP even and odd tables (see pgp-word-list.ts).
  ...
  which is the mismatch the comparison exists to surface. A throw here would only
  move that failure to a place the human never sees.

*/

import { sha256 } from '../crypto';
import { pgpWords } from './pgp-word-list';

/** Bytes of the digest a fingerprint reads — six, for 48 bits and six words. */
