/*
  ...
  Byte-twin: dfos-protocol-go/pgp_word_list.go.
...
/**
 * The EVEN table: the two-syllable words, indexed by byte value, rendering bytes
 * at even offsets (0, 2, 4, …).
 */
export const PGP_EVEN_WORDS: readonly string[] = [
...
/**
 * The ODD table: the three-syllable words, indexed by byte value, rendering bytes
 * at odd offsets (1, 3, 5, …).
 */
export const PGP_ODD_WORDS: readonly string[] = [
...
/**
 * Render octets as PGP words: byte `i` through the EVEN table when `i` is even
 * and the ODD table when `i` is odd, space-joined. Total over any input — every
 * byte value indexes both tables — so it never throws and never returns an
 * empty word.
 *
 * This is the raw encoding, over any octets. `keyWordFingerprint` is the one
 * caller the protocol defines for it.
 */
export const pgpWords = (data: Uint8Array): string =>
  Array.from(data, (byte, index) =>
    index % 2 === 0 ? PGP_EVEN_WORDS[byte] : PGP_ODD_WORDS[byte],
  ).join(' ');
