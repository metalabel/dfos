...
This is a shape the wider ecosystem has standardized twice, and the request proof sits between the two. The nearer neighbor is [HTTP Message Signatures (RFC 9421)](https://www.rfc-editor.org/rfc/rfc9421): the proof's `{method, host, path, bodyHash}` binding is, almost member for member, a 9421 signature over the components `("@method" "@authority" "@path" "content-digest")` — the digest via [RFC 9530](https://www.rfc-editor.org/rfc/rfc9530) — with `ed25519` already a registered 9421 algorithm. 9421 is nonetheless not adopted: its wire rides [RFC 8941](https://www.rfc-editor.org/rfc/rfc8941) Structured Fields plus RFC 9530 — two more parsers in five languages — for no binding this envelope's single canonical JSON payload does not already make; it defines no slot for the `credentialCID` that ties a proof to its grant; and it leaves key resolution out of scope, which is exactly the part a DID-keyed surface has to specify regardless.

The grant-exercising half of the shape is [DPoP (RFC 9449)](https://www.rfc-editor.org/rfc/rfc9449): a proof-of-possession JWS presented alongside the grant it exercises. DPoP binds `htm`/`htu` and deliberately does not cover the request body — the body binding here is a strengthening of that shape, not a restatement — and DFOS needs its own envelope rather than DPoP itself because the grant being exercised is a DFOS credential (CID-addressed, delegation-chained, revocable) and the signing key is a DID key resolved from an identity chain — the binding points are protocol objects, not OAuth ones.
...
- **Not relay surface.** The proof is never ingested by a relay, never gossiped, never content-addressed for reference. It moves in an HTTP header to an API host and dies with the freshness window. It is registered in the [extension registry](https://protocol.dfos.com/spec#extension-registry) for routing only.
...
- **`typ` is `"did:dfos:identity-proof"`** — a distinct value this spec registers in the [extension registry](https://protocol.dfos.com/spec#extension-registry), because "possession of a grant's audience key" and "possession of a bare identity's key" are different claims, and the ordinary typ-scoping rule keeps one from ever being presented as the other. A route that requires a credential rejects an identity proof at the header gate, and vice versa — no verifier ambiguity, no downgrade.
...
- **`x-dfos-actions` on the request-proof scheme is the host's action catalog**: a map of action token → human-readable description, restating the registry the host serves (for the canonical deployment, the [v0 registry above](#the-apihost-resource-and-its-actions)). Tokens are opaque strings to consumers, exactly as everywhere in this family — the catalog is documentation, not vocabulary the client interprets. The member name is shared with the [operation-level extension below](#required-actions-x-dfos-actions-on-an-operation), and the two shapes are **strict and deliberately different**: on a scheme the member MUST be a map (a catalog is a dictionary), on an operation it MUST be an array (requirements are a list) — the node's position, not its shape, says which is being read, and the shape then confirms it. A scheme member that is not a map, like an operation member that is not an array, is malformed — consumers reject it rather than guess, because lenient parsing is how divergent documents happen: one canonical shape per position keeps every consumer's parse identical.
