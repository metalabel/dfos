...
- **Pull over the existing public blob route.** A follower fetches `GET /content/:contentId/blob[/:ref]` from a source gateway — the one blob route ([WEB-RELAY.md → Content Following](https://protocol.dfos.com/web-relay#content-following)), typically pinned to an operation-CID ref, or bare `/blob` for the head. No new endpoint, no new wire field.
