...
**Why there is no `collect` action.** The principle is clean: **credentials delegate authority to others; being yourself is not a delegation.** A signer is, by definition, a holder of a key on the subject's own chain — key possession (the poll route's auth token) is necessary and sufficient to read the subject's own mailbox, and a `collect` credential would be a second, revocable-in-a-second-way statement of a fact the identity chain already states. Device de-enrollment is key rotation. The resource form therefore carries exactly one action, and a credential attenuated to `collect` on a mailbox grants nothing.
