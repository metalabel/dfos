...
A sign request is a JWS in the same envelope family as [credentials](https://protocol.dfos.com/credentials), [credit claims](https://protocol.dfos.com/content-model#credits), and revocations, with its own `typ`.
...
The rule is deliberately not [JCS (RFC 8785)](https://www.rfc-editor.org/rfc/rfc8785), the JSON canonicalization with real precedent in this problem space — the W3C [`vc-di-eddsa`](https://www.w3.org/TR/vc-di-eddsa/) cryptosuite `eddsa-jcs-2022` signs over exactly it. Three reasons: JCS would be a third canonicalization in the corpus, beside dag-cbor and the fixed-member-order rule itself; its hardest corner — ECMAScript's `Number::toString` float serialization — is dead weight for payloads that are all strings plus a bare integer; and fixed table order means the target spec's payload table _is_ the byte order, which is what lets the re-canonicalize-and-byte-compare rule above be stated in one sentence, where JCS's recursive UTF-16-code-unit key sort cannot be.
