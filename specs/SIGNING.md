...
- **Retention MUST NOT exceed the envelope's `expiresAt`**, and an expired request MUST NOT be served on any route — to the subject, the requester, or anyone. A relay MAY drop courier state earlier under its own policy; durability is explicitly not promised, and a composer that needs delivery guarantees re-deposits.
...
The relay verifies the envelope (full [algorithm](#verification-algorithm) above, wall-clock `now`), verifies the deposit credential (next section), and stores the request keyed by its CID. Responses: **201** with `{ "cid": "...", "expiresAt": "..." }`; **200** with the same body for an idempotent re-deposit of the identical token; **400** for an invalid or expired envelope; **404** when the subject is not resolvable on this relay; **403** when the credential fails the deposit rule; **413** over the caps. The aggregate deposit body MUST NOT exceed **524288 bytes** (512 KiB — a maximal credential chain plus envelope plus bundle headroom).
...
The subject is the token's `iss`. The response lists **pending** requests — deposited, unexpired, unresponded — oldest first:
...
6. The payload segment decodes to **exactly the request's target bytes** — byte equality, the whole point.
7. The signature verifies under the resolved key.

The slot is **first-write-wins**: at most one response per request, the first valid one stored. A re-put of the byte-identical token is idempotent (**200**); a _different_ valid artifact — possible when the subject holds multiple enrolled keys — is refused (**409**), and the composer reads the one that won. **201** on first acceptance. Responding to a declined request is legal (decline is advisory); the response simply wins.

#### `GET /signing/v1/requests/{cid}/response` — requester poll

**Unauthenticated.** Knowledge of the request CID is the capability: it is derived from the requester-signed payload bytes, held by the composer that deposited it and the subject that polled it, and is not enumerable (there is no route that lists CIDs). Responses:
...
SIWD is this protocol's ancestor and will become its client. The settled direction — direction, not this document's normative content: **`did:dfos:siwd` becomes a `payloadTyp`.** The SIWD challenge schema, its canonical signing input, and its verification rules survive unchanged as the target spec; what SIWD 0.1 calls the flow — carrying a challenge to wherever the key lives and carrying a JWS back — is exactly a sign request wearing a redirect. The web-redirect dance remains as one courier profile (it is a good transport for a browser already standing at the platform's door); the localhost-CLI sovereign path is replaced outright by the mailbox, which does the same job without a port, a health probe, or a same-machine constraint; scope-to-credential issuance is untouched. That rewrite is a follow-up to this spec, not a rider on it: it lands once the envelope has shipped vectors and a running mailbox behind it.
