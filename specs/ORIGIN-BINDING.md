...
Verifiers MUST fetch over HTTPS with ordinary TLS validation, MUST NOT follow redirects, and SHOULD bound the read: a conforming body is under a hundred bytes, so a small cap (e.g. 1024 bytes) rejects garbage before reading it.

**A redirect is a non-answer, never a contradiction.** The attestation is the named origin speaking for itself at the registered path, and a redirect is that origin declining to answer there: whatever the redirect points at — another origin, or another path on the same one — the bytes would arrive from somewhere the binding did not send the verifier. A verifier that receives any `3xx` therefore treats the path exactly as it treats absence: the file attests nothing, the app-description fallback (below) applies as it does on a `404`, and if no channel answers, the verdict is the silent one (`stale`, [below](#verification)) — never `broken`, because nothing contradicted anything. This is a deliberate divergence from the [atproto handle resolution](https://atproto.com/specs/handle) the attestation shape is copied from, which permits _"HTTP redirects (eg, 301, 302) … up to a reasonable number of redirect hops"_: followed hops make the effective serving path unauditable, a browser-resident verifier cannot distinguish a same-origin hop from a cross-origin one before following it, and no-redirects is one byte-comparable rule with no origin-equality edge cases. The DNS method exists precisely for the operator this rule touches — a domain whose HTTP serving it does not control attests through a TXT record instead, and either channel alone suffices.

**App-description fallback.** If `/.well-known/dfos-did` yields a **non-answer** — a `404`; a redirect, which is absence in everything but status code (above); or a `200` whose trimmed body is not exactly one DFOS DID, the shape a host serving its application shell for every unknown path produces — a verifier MUST fall back to fetching the [SIWD app description](https://protocol.dfos.com/siwd) at `/.well-known/dfos-app.json`, under the same no-redirects rule: a structurally valid document whose `client_did` names the candidate DID attests it, exactly as the well-known file would. This makes every existing SIWD application attest-back-capable with zero changes — the origin already publishes its DID; this spec just reads it. The fallback applies only on non-answers: a body that **is** exactly one DFOS DID naming a different DID is a contradiction (below), and MUST NOT be fallen through. The split is safe by construction — a page that is not a DID can silence the channel, never break the binding, so garbage served at the path degrades the verdict to unchecked rather than contradicted.

**Failure is not absence.** A query that fails — a network error, a TLS failure, a timeout, a server error — tells a verifier nothing about the path, so it is not a non-answer and a verifier MUST NOT fall back on it: the HTTPS channel is simply silent, and silence earns the verdict it earns ([below](#verification)). A non-answer is the named origin answering at the named path with something that attests nothing; a failure is that origin not answering at all, and a binding verdict follows what a domain publishes, never how well it was serving in the minute it was asked.
...
| **stale**  | The chain names the domain and the domain is **silent** — no TXT record, and a non-answer at both well-known paths (absence, a redirect, or a body that is not a DID / not a valid app description), or the queries fail (network error, TLS failure, timeout, server error) |
