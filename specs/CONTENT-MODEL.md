...
> **Schema stability.** `post/v1` evolves only additively; any breaking change to this shape is a `post/v2`, never an in-place edit. One consequence worth stating plainly: the example post documents inside the [protocol spec](https://protocol.dfos.com/spec)'s test-vector section — and the matching `examples/` chain fixtures in the protocol package, which are the same deterministic artifacts — carry an older draft credit shape and do not validate against `post/v1`. That is deliberate and not a defect to fix. Those vectors are CID-frozen: their exact bytes are the input to a published hash, and a document CID appears verbatim in the spec's reference-artifact tables, so re-minting them would invalidate every published vector downstream of them. They are read as fixtures for the signing and verification rules they demonstrate — chain linkage, CID derivation, signature verification, all of which are indifferent to what the document says — never as current content-model examples. A drift guard in the protocol test suite pins the fixtures and the spec vectors to each other, so the two can never disagree about the bytes.
...
**Delegated writes are the multi-writer mechanism, not the attribution mechanism.** A design worth rejecting explicitly: issuing the credited DID a write credential so it can append a no-op `update` re-committing the head `documentCID`. That pattern grants real chain authority just to obtain a signature, leaves permanent public proof-plane residue for content that may be gated, and cannot name which credit it is claiming — the credit claim exists so that none of that is necessary. Write credentials and delegated content operations are specified in [CREDENTIALS.md](https://protocol.dfos.com/credentials) for what they are actually for: a chain with **more than one writer** (collaborative documents, an editor with commit rights, a device writing for its owner, a service maintaining an index). That capability is load-bearing; it is simply the wrong instrument for saying who made a thing.
