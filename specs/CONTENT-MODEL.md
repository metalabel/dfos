...
7. **Resolve the claimant identity** named by `payload.did` and find the key named by the `kid` fragment among every key that identity chain has ever held. A claim has no [basis time](https://protocol.dfos.com/spec#time-basis) and no temporal check. Unresolvable identity resolves **unverifiable**. Resolvable identity with no such key resolves **invalid**.
...
An implementation MUST resolve claimant keys from the identity chain's full key history, not from its current state. A resolver that returns only current-state keys resolves **invalid** for every claim signed before the claimant's most recent rotation, which silently un-credits real work: a claim signed in 2026 does not evaporate because the claimant rotated keys in 2028.
