...
**Credits.** Each credit is `{ did, role?, name?, claim? }`: `did` (REQUIRED) is the credited identity; `role` (OPTIONAL) is a free-text role — open vocabulary, not an enum (`"author"`, `"editor"`, `"photography"`, …); `name` (OPTIONAL) is the credited party's display name at time of publication, a rendering convenience that is never authoritative (the credited DID's profile is); `claim` (OPTIONAL) is a `did:dfos:credit-claim` JWS in which the credited DID asserts this same credit itself. Array order is display order, and the **first entry is the primary author**. Omit `credits` entirely for unattributed content (system or imported posts). A bare entry is the **assertion tier** of the [authorship lattice](#authorship): the operation signer asserts it, and the protocol verifies the signer, never the credit. Adding `claim` upgrades the entry to the proof tier — see [CREDITS.md](https://protocol.dfos.com/credits) for the envelope, the two-way bind on `(contentId, did, role)`, and the four verification states.
...
> **Pre-adoption amendments (2026-07).** This shape is the product of three breaking amendments made **in place**, deliberately without minting `post/v2`, all inside the pre-adoption window. The first replaced an earlier draft — a `createdByDID` string in place of `credits`, and a legacy `{ id, uri? }` media shape predating the [Media object](#media-object) — while zero `post/v1` documents existed on any chain. The second removed `topics`, narrowed `format` to the two post formats, and added `publishedAt`. The third (this revision) renamed the credit entry's `label` to `role` and added the optional `name` and `claim` fields, making the entry the embedding site for a [credit claim](https://protocol.dfos.com/credits) — the credit entry needed one unambiguously-named role field because `role` is a component of the claim's byte-exact bind, and the previous entry shape was closed to additional properties, so a `claim` field could not have been added without amendment. For the second and third amendments alike, every existing `post/v1` document lived only on shadowed (never-served) chains that were wiped and re-anchored under the amended shape — equivalent, from any external observer's standpoint, to the documents never having existed. **The window closes for good when post chains are first publicly served**: from that point `post/v1` evolves only additively, and any further breaking change is a `post/v2`. One consequence worth stating plainly: the example post documents inside the [protocol spec](https://protocol.dfos.com/spec)'s test-vector section — and the matching `examples/` chain fixtures in the protocol package, which are the same deterministic artifacts — illustrate the PRE-amendment credit shape and do not validate against the amended `post/v1`. That is deliberate and not a defect to fix. Those vectors are CID-frozen: their exact bytes are the input to a published hash, and a document CID appears verbatim in the spec's reference-artifact tables, so re-minting them under the amended shape would invalidate every published vector downstream of it. They are read as fixtures for the signing and verification rules they demonstrate — chain linkage, CID derivation, signature verification, all of which are indifferent to what the document says — never as current content-model examples. A drift guard in the protocol test suite pins the fixtures and the spec vectors to each other, so the two can never disagree about the bytes.
...
The document says who made it: the `credits` array in `post/v1` (and analogous authorship fields in other schemas). Costless and unverified — liner notes. The trust anchor is the operation signer: _the identity that signed this revision asserts these credits._ Nothing more is claimed, and consumers SHOULD display it as an assertion of the signer, not a verified fact. This is often exactly right: a custodial host or space signs content on behalf of the people it credits. An entry that stops here is **unclaimed**, which is the ordinary case and not a defect.

### 2. Proof — the credit claim

A credited DID converts its credit into a verifiable fact by signing the credit itself: a **credit claim**, a small standalone JWS asserting `(contentId, did, role)`, carried in the entry's `claim` field. The full envelope, the two-way bind, the four verification states (**claimed**, **unclaimed**, **invalid**, **unverifiable**), and the verification algorithm are specified in [CREDITS.md](https://protocol.dfos.com/credits).

Two properties matter at this altitude:

- **The claim lives in the document, not on the proof plane.** It is not an operation and is not gossiped — it is bytes inside the document blob, so a credit on gated content is exactly as visible as that content. Attribution never leaks the shape of private work.
- **It binds the chain, not a document.** A claim survives every subsequent revision verbatim, and the bind names the `role`, so a claim says which credit it is claiming.

**Claims are durable, and deliberately so.** A claim is not gossiped, but it is still a signed statement inside bytes that may already be copied — re-signing nothing un-signs it, and there is no renunciation mechanism (`did:dfos:credit-renounce` is reserved and undefined). What a claimant can do without any new primitive is stop claiming: an entry whose `claim` is dropped on the next revision is simply **unclaimed**, an ordinary state, while the operation log preserves the earlier revision so nothing is falsified. Deployments SHOULD still be plain about permanence at the moment of claiming.

**Delegated writes are the multi-writer mechanism, not the attribution mechanism.** An earlier draft of this section described a _claim operation_ — issuing the credited DID a write credential so it could append a no-op `update` re-committing the head `documentCID`. That pattern is retired for attribution: it granted real chain authority just to obtain a signature, left permanent public proof-plane residue for content that may be gated, and could not name which credit it was claiming. Write credentials and delegated content operations remain exactly as specified in [CREDENTIALS.md](https://protocol.dfos.com/credentials) and are unchanged — they are how a chain supports **more than one writer** (collaborative documents, an editor with commit rights, a device writing for its owner, a service maintaining an index). That capability is load-bearing; it was simply the wrong instrument for saying who made a thing.

### 3. Sovereign proof — reserved

When authors hold their own keys, the same credit claims apply unchanged — the proof tier simply gains the strength of self-custody, since the claimant signing its own credit _is_ the self-custody case. No additional vocabulary is defined yet; future extensions are additive.
