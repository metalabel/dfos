...
| `timestamp`          | Creation time in the [timestamp grammar](https://protocol.dfos.com/spec#timestamp-grammar), floor-normalized to whole seconds (`.000Z` millisecond component), as [SIWD](https://protocol.dfos.com/siwd#challenge-schema) normalizes its challenges.                                                                                                                                                                                                               |
...
- `prevCID` binds the proof to one chain position. The envelope proves consent to _this_ introduction, built on _this_ head — not to the key's presence in the chain at large. A removed key cannot be re-added on the strength of an old envelope: the re-adding operation has a different `previousOperationCID`, so the old proof does not cover it, and only the holder can mint the new one. Consent does not stand; it is spent at the position it named.
...
- `typ` — exactly one registered purpose value from the [purpose registry](#purpose-registry) below. Verifiers reject any other `typ`; the gate is what keeps a proof signed for one purpose from ever being presented under another. Within a purpose, it is [position binding](#position-binding) — the `did`, `roleSet`, and `prevCID` byte-equality of [Presentation Verification](#presentation-verification) step 5 — that keeps a proof signed for one ceremony from being presented for another.
...
- **The controller-verified leg.** A chain's own controller introduces a key its human's custody can reach — no host mediates. The controller's tool mints the nonce and delivers `{did, roleSet, prevCID, nonce}` to wherever the candidate key lives; the key signs and returns the envelope; the controller's tool verifies and appends. Here `audience` is the target chain's DID, and the verifier REQUIRES `audience` to byte-equal the payload's `did`. The device gap is the general case, not a requirement: a tool rotating a chain onto a key it minted itself signs the same envelope wearing both hats, and the introduction is proved by the same rule whether the candidate key sits across a room or in the same keystore.
...
The carriage conveys exactly two values — **an authority and a code** — and the code is the ceremony's identifier: it selects the ceremony at resolution and travels beside the envelope at presentation. What the carriage does not carry is the signing context: everything the payload binds and everything the holder's human must see lives in the _resolution_ of a live code, so every carriage form funnels through one resolution step, and a tool MUST resolve before signing — there is no self-contained carriage that skips it. The code itself names no identity; resolving it names everything — this is deliberate, and its trust posture is examined under [Security Considerations](#security-considerations).

**Short code.** The human-typeable display form, `<authority>/<code>`, where `<code>` is an operator-chosen compact token. The token is operator-minted, but its floor is stated here for every ceremony operator, because a code is typed by humans and guessed by machines — the tension [RFC 8628 §6.1](https://www.rfc-editor.org/rfc/rfc8628#section-6.1) resolves for the device authorization grant's user code, whose answers this floor takes:

- **Charset.** Codes SHOULD be drawn from the 32-symbol alphabet `ABCDEFGHJKLMNPQRSTUVWXYZ23456789` — uppercase letters and digits with the confusable glyphs (`0`/`O`, `1`/`I`) excluded, so a code survives being read aloud, handwritten, or typed from a photo. One delta from RFC 8628's example charset is deliberate: 8628 drops vowels _"to avoid randomly creating words"_, while this alphabet keeps `A`, `E`, `U`, `Y` for the larger symbol space; an operator that prefers the no-words property MAY narrow further, since the code is opaque to every holder tool.
- **Length.** Codes SHOULD carry at least 8 characters — 32^8 ≈ 2^40 possibilities, above RFC 8628's 20^8 ≈ 2^34.5 example — and an operator choosing a different charset SHOULD hold the entropy at or above that line.
- **Grouping punctuation belongs to the display, never to the code.** A code is read off one screen and entered on another, so it arrives as displayed: an operator SHOULD strip characters outside the code alphabet (spaces, dashes, and other grouping marks) and normalize case before comparison — RFC 8628 §6.1's server-side rule, and the reference tooling's existing behavior. No such mark is in the alphabet, so stripping can never collide two codes: `ABCD-2345`, `abcd 2345`, and `ABCD2345` name one code.
- **Lifetime and rate.** A code lives exactly as long as its ceremony (the reference operator's ceremonies live 10 minutes), and an operator SHOULD rate-limit resolution and presentation attempts per code and per source, so the entropy above is measured against a bounded number of guesses rather than an open faucet. Single-use is already the rule: a code is consumed with its ceremony ([below](#carriage)).
...
**Resolution.** A tool resolves with `GET https://<authority>/.well-known/dfos-key-proof?code=<code>`. `dfos-key-proof` is a suffix minted in [RFC 8615](https://www.rfc-editor.org/rfc/rfc8615)'s well-known namespace, whose §3 requires registering new suffixes in the IANA Well-Known URIs registry — the [extension registry's external-registrations table](https://protocol.dfos.com/extensions#external-registrations) tracks it. A live code answers the full signing context:
...
- **One key, one DID — ever.** A holder SHOULD refuse to sign a key proof for a key that any identity's chain has ever declared _and proved_ (its own DID included, for a key-add naming a different identity). The [`key=` reverse index](https://protocol.dfos.com/web-relay#identities-get-indexv0identitiesdidkeyhaspublicprofilenamecontainsorderafterdidlimitn) is has-ever-proved across all three key sets, and its rows survive rotation and deletion — proving one key into two chains publishes an irreversible public link between them. An _unproved_ declaration of the key in some other chain is not a link and not a burn: it is void, it never indexes, and it never obligates the true holder ([PROTOCOL.md → Key Possession](https://protocol.dfos.com/spec#key-possession)). Reference tooling checks against a named oracle relay and refuses by default. A holder with no oracle of its own takes the one the [carriage resolution names](#carriage), for that ceremony only.
...
- **Comparable fingerprints.** Wherever a human is asked to compare a key across two surfaces — the holder's tool printing the key it presented, and a ceremony surface displaying the key that arrived — both SHOULD render the same **word fingerprint**: the first six bytes of `SHA-256` over the key's multikey string (the UTF-8 bytes of the `z…` form), each byte rendered through the [PGP Word List](https://en.wikipedia.org/wiki/PGP_word_list) — the even list for bytes at even offsets, the odd list for odd offsets, the alternation the list was designed with so a transposed pair cannot read correctly. A multikey prefix is not a fingerprint humans compare; six words are, and a fingerprint nobody compares defends nobody. The multikey string itself remains the identifier everywhere bytes are matched — the word form exists for eyes, never for verification.
...
- **Carriage interception** — possession of a code or QR yields the ability to _resolve_ the ceremony's context and to _attempt_ a presentation, which still requires signing with a key the interceptor does not hold; the operator's own ceremony authorization is the gate on everything after. The resolution names the adopting identity — that disclosure is deliberate: the named facts (DID, handle, display name) are public identity facts, the code is a short-lived single-ceremony capability, and a holder who cannot see whom they are joining cannot meaningfully consent. The trade is stated, not hidden: a code-holder learns _which_ public identity is mid-ceremony, and nothing else.
