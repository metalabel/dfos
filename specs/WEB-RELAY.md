...
**This family defines no `order`.** Enumeration is the lexical default only — credential `cid` ascending — and there is no time ordering to select. An `order=` on this route is ignored rather than rejected: it is not a `400`. That is a statement about this family only — this spec defines no general rule for parameters a route does not define, and the [boolean fail-closed rule](#determinism-and-coverage) shows the treatment is per-parameter rather than uniform. The `400` the identity, content, artifact, countersignature, and operation families return for `order=` is for an **invalid value of a parameter those routes define**, which is a different rule and not one this route can apply to a parameter it does not have. One consequence is worth naming because it is not obvious: the `order=`-probe a client uses to feature-detect time ordering elsewhere returns no verdict here, so a client cannot distinguish this family's deliberate absence of ordering from a relay that ignores the parameter.
