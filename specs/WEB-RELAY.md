...
- **`GET /.well-known/dfos-relay`** — discovery (RFC 8615) lives at the root by convention; it announces the relay's own release version and its capability flags. (The proof plane's version is not a field here — it is encoded in the frozen route prefix `/proof/v1`.)
...
| `version`              | string  | The relay's own release version (semver). This is the relay implementation's version, independent of the frozen proof-plane clock — the proof plane's version is encoded in its route prefix (`/proof/v1`), not here |
...
| `capabilities.content` | boolean | Whether the relay supports content plane (blob upload/download). The documents endpoint is part of the content plane and rides this same flag                                                                        |
...
This response is **frozen with protocol v1** — it is the pure replayed chain-state projection and carries no derived authorization data. Public-read discovery (surfacing the `aud: "*"` credentials that currently authorize `read` on a chain, re-verified live) is a **document gateway** concern on the `0.x` clock, not a field on this frozen route. See [DOCUMENT-GATEWAY.md → Public-read discovery](https://protocol.dfos.com/document-gateway#public-read-discovery-0x).
...
The documents endpoint is part of the content plane: when `capabilities.content: false`, this route returns **501 Not Implemented** (`content plane not available`). It rides `capabilities.content` and has no separate capability flag of its own.
