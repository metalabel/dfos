...
`key=` is the reverse lookup "which identities has this key ever been declared by." The value is matched **byte-for-byte** against the multibase public-key strings carried in the key arrays (`authKeys`, `assertKeys`, `controllerKeys`) of the chain's **accepted operations — every operation, not just the current head**: an identity matches when any accepted genesis or update ever declared the key, whether or not a later update rotated it out. The filter is deliberately **has-ever-declared** rather than current-state. Its consumers are audit ("what has this key ever controlled") and key-loss recovery, and both need the full declaration history: a holder rediscovering identities from a restored key may hold exactly the keys that later updates rotated out, so a current-state match would hide the chains that matter most — while current state is always one fetch away, per [Hints, Not Authority](#hints-not-authority). One key may match many identities, and a deleted identity still matches — the row's `isDeleted` field records the deletion, and a consumer that does not want deleted rows filters on that field client-side (this route defines no `isDeleted` parameter). There is no key-class or role column: which array carries the key, and whether it is current, is the chain's answer, not the index's. Like `signer=` on the content family, this is an **actor-axis verification outcome**, proof-tier — a declared key is the material through which an actor acts, the match is computed from accepted, signature-verified operations alone (no held-bytes dependence), and it reveals nothing not already derivable from the public per-chain log. The value is matched as an opaque string: a string no operation ever declared simply matches nothing — no format validation, no `400`.
