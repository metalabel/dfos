...
**Enumeration and resolution are separate promises for deleted identities.** A relay **MAY** omit deleted (`isDeleted: true`) identities from the **enumeration** shapes of this route — the unfiltered listing and every filtered variant of it (`hasPublicProfile`, `nameContains`, `key`, `order`) — serving only live chains there. A relay **MUST** still resolve a deleted identity through the **point lookup** `did=`, returning its row with `isDeleted: true`. The reason is arithmetic: an operator who has sealed a large fraction of their chains would otherwise have no way to let a caller enumerate the live ones, since the route defines no `isDeleted` filter to exclude them with — so the choice would be between serving an unusable listing and not serving the route.

The line is drawn at **the caller naming the identity**. Enumeration is a browse surface and a relay is sovereign over what it lists; resolution answers a question about a specific DID, and answering it with an empty page would be a relay asserting non-existence, which no relay is ever entitled to do. `GET /proof/v1/identities/:did` on the frozen proof plane is unaffected and resolves a sealed chain regardless.

Two things a consumer must therefore not assume. **Absence from a listing is not deletion** — it was already not proof of absence ([Hints, Not Authority](#hints-not-authority)), and this makes one of the reasons concrete. And **`key=` recovery is enumeration**: on a relay that omits deleted identities, the has-ever-proved lookup surfaces only the live chains a key proved into, so a holder rediscovering a sealed identity resolves it by `did=` or reads the proof plane directly. A relay that enumerates deleted rows keeps the fuller answer; both behaviors are conformant, and a caller that needs the fuller one asks a relay that gives it.

`key=` is the reverse lookup "which identities has this key ever been proved into." The value is matched **byte-for-byte** against the multibase public-key strings whose memberships entered the chain's **effective key state** ([PROTOCOL.md → Key Possession](https://protocol.dfos.com/spec#key-possession)) under any of the chain's **accepted operations — every operation, not just the current head**: an identity matches when its genesis key is the key, or when any accepted update ever introduced the key with a valid possession proof, whether or not a later update rotated it out. The filter is deliberately **has-ever-proved** rather than current-state — and proved rather than declared: a **void** membership (a key some chain listed without its holder's proof) never indexes, so a hostile or defective listing neither surfaces the chain in the key's recovery results nor burns the key against its true holder's own use. Its consumers are audit ("what has this key ever controlled") and key-loss recovery, and both need the full proved history: a holder rediscovering identities from a restored key may hold exactly the keys that later updates rotated out, so a current-state match would hide the chains that matter most — while current state is always one fetch away, per [Hints, Not Authority](#hints-not-authority). One key may match many identities, and a deleted identity still matches on a relay that enumerates deleted identities at all — the row's `isDeleted` field records the deletion, and a consumer that does not want deleted rows filters on that field client-side (this route defines no `isDeleted` parameter; see the enumeration-vs-resolution rule above for the relays on which the client-side filter has nothing to do). There is no key-class or role column: which array carries the key, and whether it is current, is the chain's answer, not the index's. Like `signer=` on the content family, this is an **actor-axis verification outcome**, proof-tier — a proved key is the material through which an actor acts, the match is computed from accepted, signature-verified operations and their embedded proofs alone (no held-bytes dependence), and it reveals nothing not already derivable from the public per-chain log. The value is matched as an opaque string: a string no operation ever proved simply matches nothing — no format validation, no `400`.
...
**This family defines no `order`.** Enumeration is the lexical default only — credential `cid` ascending — and there is no time ordering to select. An `order=` on this route is therefore an unrecognized parameter and is ignored, the same as any other unrecognized parameter on any index route; it is not a `400`. The `400` the identity, content, artifact, countersignature, and operation families return is for an **invalid value of a parameter those routes define**, which is a different rule and not one this route can apply to a parameter it does not have. One consequence is worth naming because it is not obvious: the `order=`-probe a client uses to feature-detect time ordering elsewhere returns no verdict here, so a client cannot distinguish this family's deliberate absence of ordering from a relay that ignores the parameter.
