...
**Content chains fork; identity chains do not.** This section is the content-chain rule; the identity-chain rule is [Identity Linearity and Admission](#identity-linearity-and-admission) below.
...
An identity chain is linear per view (PROTOCOL.md "Views"). A relay keeps the log it serves linear at ingest:

**Conflicting extension → permanent rejection.** An incoming identity operation whose `previousOperationCID` references an operation that already has a committed child is refused with the named error `identity chains are linear: conflicting extension refused`. This is a **permanent rejection**, not a dependency failure: it is not buffered, not retried, and never admitted later — first-seen wins locally, whatever the competing operation's `createdAt` claims. The rule holds on every path an identity operation arrives by — direct submission, gossip, sync, and read-through. A peer-log identity operation that conflicts with a locally-committed extension is refused identically: committed order in this relay's log is never auctioned or re-arbitrated. The refusal is this relay's admission verdict and not a claim that the refused operation is invalid: two operations at one chain position are two views of the identity ([PROTOCOL.md, Views](https://protocol.dfos.com/spec#views)). (Historical admission of committed logs is otherwise unchanged — see [Key Resolution](#key-resolution).)

**Possession status never gates admission.** The relay admits and sequences every structurally valid identity operation — linearity, CIDs, timestamps, declared-state signer validity — whatever its key proofs say; this is the relay's door of the three-door rule, normative in [PROTOCOL.md → Key Possession](https://protocol.dfos.com/spec#key-possession). An update introducing a key without a valid proof ingests exactly like any other: the membership it claims is **void** in every projection the relay serves (state resolution, the [`key=` index](#identities-get-indexv0identitiesdidkeyhaspublicprofilenamecontainsorderafterdidlimitn), signer resolution for consumers), but the operation itself is committed log. Rejecting it instead would make log membership depend on semantic validation — and on a branchless linear chain, two relays disagreeing about one operation's proofs is a forked log, a split-brain identity, and a permanent stall under union-of-CIDs sync. The log answers what was written; the projections answer what counts.

**Admission order is local.** No rule in this corpus arbitrates between relays that admitted different successors at one chain position. Each serves its own view, and which view a reader gets is a choice of relay ([PROTOCOL.md, Views](https://protocol.dfos.com/spec#views)). What first-seen admission buys locally is a log that is never re-arbitrated by timestamp: a once-current key cannot bid a committed successor out of this relay's log, whatever its `createdAt` claims. An identity write this relay refuses is admitted by a relay that holds no successor at that position, so the writer's remedy is to retry here or to continue the chain there.
...
| `stats.opCount`            | number         | OPTIONAL additive telemetry. Entries the relay **holds** in its global operation log. On a relay that prunes or [culls](#operation-log) this is below the log's tip position and is not a proxy for it — a consumer that diffs `opCount` against a cursor or sequence number is comparing two different quantities. Counts are relay-local and not comparable across relays                                                              |
| `stats.countsByKind`       | object         | OPTIONAL additive telemetry. Global-log counts by primitive kind — **operations, never chains**: the `identity` bucket counts identity operations, not identities. Chain counts are not served anywhere; they are reached only by enumerating the index. Reference relays emit `identity`, `content`, `artifact`, `credential`, `countersign`, `revocation`                                                                              |
...
`capabilities.write` and the lite node described below are **two different questions**, and the flag answers only the first. `capabilities.write` says whether this relay accepts writes over HTTP. It says nothing about where the relay's corpus comes from. A relay MAY be an **origin** — the authoritative source of its own log, minting operations out of band from whatever system of record it runs, and refusing external writes precisely because nothing outside it is entitled to append. `write: false` with `ingestion: "closed"` is exactly as legible on such an origin as on the pull-only mirror below: it means the same thing (no HTTP write route), and it implies nothing about the relay being downstream of a peer. An origin holds the whole corpus by construction, which is the opposite posture from a lite node's, and neither is derivable from the flag. The two axes compose: `capabilities.write` is the **admission surface**, `ingestion` is the **admission mode**, and where a relay's operations come from is a deployment fact the wire does not carry.
...
**A relay's log is its own, and no relay's log is the corpus.** Append-only names the relay's serving discipline — entries are appended in ingestion order and never reordered — not a promise that every relay holds everything forever. A relay is sovereign over its own retention; peers MUST NOT treat any relay's log as complete, and cross-relay operation counts are not comparable. What federates truth under that honesty is the artifact discipline, not log completeness:
...
- **Culling by visibility is sanctioned retention.** A relay MAY hold only the operations of chains that are publicly readable and cull the rest, pruning around revocations per the rule above. This is retention, not admission: content visibility is not a property of a content operation at all — a chain is created, then made publicly readable later by a separate `aud: "*"` credential — so no [admission policy](#ingestion-admission) can express "I host only public content", and the stance is reached by what a relay keeps rather than by what it accepts. Two consequences are visible on the wire and are the relay's to own, not defects to be read as inconsistency:
  - **Revocations outlive what they revoke.** A captured grant reads **revoked**, never unknown, because the revocation is retained after the credential it retracts is gone. That is the discipline working: the retraction record is the part that must survive.
  - **The log has holes, and `stats.opCount` is not a log position.** Culled operations leave permanent gaps in the relay's sequence. `opCount` counts operations **held**, so on a culling relay it is far below the position of the log's tip and is not a proxy for it. A consumer that diffs `opCount` against any cursor or sequence number is comparing two different quantities.
...
A reader finds a content-plane host through the identity's `services` vocabulary ([PROTOCOL.md → Services](https://protocol.dfos.com/spec#services)). Two open-namespace service types serve it — additive, requiring no protocol or relay change, both indexed in the [extension registry](https://protocol.dfos.com/spec#extension-registry):
...
The two read shapes carry a known privacy trade, stated here because the family ships both halves. `GET /revocations/v1/credential/:credentialCID` is OCSP-shaped ([RFC 6960](https://www.rfc-editor.org/rfc/rfc6960)): a per-credential status question that tells the relay exactly which credential a verifier is checking. `GET /revocations/v1/issuer/:did` is CRL-shaped ([RFC 5280 §5](https://www.rfc-editor.org/rfc/rfc5280#section-5)): fetch the issuer's whole set and check locally, disclosing nothing per-check at the cost of the transfer. That correlation-resistance trade is the one [W3C Bitstring Status List v1.0](https://www.w3.org/TR/vc-bitstring-status-list/) (a W3C Recommendation) exists to dissolve with a herd-privacy bitstring — not adopted here, because it requires issuer-allocated status indices and a published, maintained status credential, which is incoherent for ad-hoc CID-addressed credentials minted at consent moments with no allocator. A verifier that weighs the trade picks its half: the issuer feed is the herd-privacy read this surface already has.
...
- **Maintenance.** The index is fully re-derivable from the operation log plus held blobs. Reference implementations maintain it incrementally and expose a rebuild path for pre-existing corpora; either way the serving contract is identical. Where the incremental work runs is an implementation choice the wire does not carry.
...
Parameters: `kind` (optional exact match, one of `identity-op`, `content-op`, `artifact`, `countersign`, `revocation`, or `credential`; `400` otherwise), `chainId` (optional exact match against the operation-log routing identifier), `signerKey` (optional exact multibase public-key match against the key the row's verified signature resolved to at ingest; an opaque byte match with no format validation and no `400`, the [identities `key=`](#identities-get-indexv0identitiesdidkeyhaspublicprofilenamecontainsorderafterdidlimitn) posture), `order` (`createdAt.desc` or `ingestedAt.desc`, default `ingestedAt.desc`), `after` (the opaque ordered cursor), and `limit` (default 100, max 1000). Filters are ANDed.

The signer filter is **key-addressed, not DID-addressed** — a proof-tier actor-axis filter over every row kind. A `kid` is an identity-local indirection, rebindable across key material by the chain that defines it; the public key a signature verified against is the immutable fact of the row. Ingestion already resolves `kid` to that key to verify the operation, so the row retains it as stored metadata — the filter reads what verification already computed, and nothing re-decodes the corpus. A DID-addressed signer filter is deliberately not defined on this route: the DID behind a key at any moment is the identity index's question ([`key=`](#identities-get-indexv0identitiesdidkeyhaspublicprofilenamecontainsorderafterdidlimitn)), and answering it per-row would put an assertion-tier join inside a proof-tier filter.
...
| Field                  | Type           | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `opCount`              | number         | Operations stored for this chain — identity chains are linear per view, so this is the linear operation count (content-chain `opCount` in the content family below remains branch-inclusive)                                                                                                                                                                                                                                                                                                                            |
| `genesisAt` / `headAt` | string         | Author-claimed `createdAt` of the genesis and current head operations                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `profile`              | object \| null | The [well-known projection](#well-known-projections), or `null` when the identity declares no profile-labeled content-chain anchor. Non-null means the chain **names** an anchor — never that this relay holds it                                                                                                                                                                                                                                                                                                       |
| `profile.anchor`       | string         | The anchored contentId — the client's verification pointer. This is a chain fact, read from the identity's own signed services; it is present whenever the identity names an anchor, **including when this relay holds nothing under that contentId**                                                                                                                                                                                                                                                                   |
| `profile.publicRead`   | boolean        | Whether a standing public-read grant currently authorizes anonymous read of the anchored chain, per this relay's fold — a hint, never an access decision. `false` is the fold's answer for a chain this relay does not hold at all, so it is not a claim that a grant is absent, only that this relay would not serve the bytes anonymously                                                                                                                                                                             |
| `profile.docSchema`    | string \| null | `$schema` declared by the held head document. **MUST be `null` whenever the relay does not hold the anchored chain's current head bytes** — including when it holds no such chain at all. `null` is the honest unknown of [coverage](#determinism-and-coverage) and has three causes: bytes not held, bytes that do not decode, and a decoded document declaring no string `$schema`. A non-null value is a claim that this relay holds and decoded those bytes; a `null` is never a claim about what the document says |
| `profile.name`         | string \| null | Extracted per the projection table; `null` on any circuit breaker — including when the relay does not hold the anchored chain's head bytes (there is nothing to extract from), and including when `profile.publicRead` is `false` (a non-public profile never projects its name)                                                                                                                                                                                                                                        |

**A named anchor and a held anchor are different facts.** `profile.anchor` comes from the identity chain; everything beside it comes from what the relay holds under that contentId. The two are routinely out of step, and the row says so rather than hiding it: an identity that anchored a profile and later withdrew it — or whose anchored chain this relay never held, pruned, or [culled](#operation-log) — keeps a non-null `profile` carrying its `anchor`, with `docSchema: null`, `name: null`, and `publicRead: false`. That row is the honest rendering of "this identity names a profile; ask elsewhere for it". Read `docSchema: null` as the honest unknown it is, in **both** directions: not-held always produces it, and it does not imply not-held — undecodable bytes and a held document declaring no string `$schema` reach the same `null`, and none of the three is a claim about the document's content. A consumer that needs to separate them fetches the chain. Because `hasPublicProfile` folds no-anchor, private-anchor, and not-held-anchor into a single `false`, a consumer that needs to tell them apart reads the row's `profile` object, not the filter.

Parameters: `did` (optional exact DID match, returning zero or one row; composes with the remaining filters), `key` (optional exact match against a public key the identity has **ever declared** — semantics below; composes with the remaining filters), `hasPublicProfile` (optional boolean filter on the predicate "`profile` is non-null AND `profile.publicRead` is true" — `true` keeps only rows where it holds, `false` keeps only rows where it does not, absent applies no filter), `nameContains` (optional case-insensitive substring filter over projected `profile.name`; non-authoritative/amber; applied before keyset pagination), `order` (optional time ordering — `genesisAt.desc` or `headAt.desc`; `400` on any other value), `after` (a `did` keyset cursor in the lexical default — returns rows with `did` strictly greater — or an opaque token in ordered mode), `limit` (default 100, max 1000). Multiple profile-labeled anchors resolve deterministically to the one with the lexicographically smallest service `id`.

**Discovery and resolution are separate promises for deleted identities.** A relay **MAY** omit deleted (`isDeleted: true`) identities from the **discovery** shapes of this route — the unfiltered listing, the keyset and ordered walks, `nameContains`, and `hasPublicProfile` — serving only live chains there. A relay **MUST** return a deleted identity, carrying `isDeleted: true`, from either **resolution** shape: `did=` and `key=`.

**`did=` or `key=` makes the request a resolution, whatever else it carries.** The precedence is on the presence of the parameter, not on the rest of the query: `did=X&hasPublicProfile=true` and `key=K&order=headAt.desc` are resolutions and MUST return the sealed row if it satisfies the other filters. A relay MUST NOT let a discovery-shaped filter reintroduce the exclusion, or the guarantee would evaporate exactly when a caller narrowed a lookup they already had the identifier for.

The line is **whether the caller already holds the identifier**, not how many rows come back. Discovery is a browse surface and a relay is sovereign over what it lists. Resolution answers a question about material the caller possesses, and neither of these two is a way of learning identifiers you did not already have: a DID is the identifier, and a public key is the material an identity acts through. Answering either with an empty page would be a relay asserting non-existence, which no relay is entitled to do — and a sealed chain stays addressable forever, because it is the verification substrate for everything that identity ever signed elsewhere.

`key=` is the harder half of that rule and the reason it is a MUST rather than a SHOULD. Its two consumers cannot fall back to `did=`, because neither has a DID:

- **Key-loss recovery.** A holder who has restored a seed derives public keys and holds no identifier at all. "Resolve it by `did=` instead" is not an instruction they can execute. A relay that hides sealed rows here tells a returning holder their identity never existed.
- **Mint-time burn checking.** A minter asks whether a key it is about to declare has ever been proved anywhere, and refuses when rows come back. Hidden sealed rows answer "never proved" for a key a sealed chain did in fact prove, so the key is re-minted — the precise outcome the has-ever-proved semantics exist to prevent. An omission here is not a thinner answer; it is a wrong one.

`GET /proof/v1/identities/:did` on the frozen proof plane is unaffected and resolves a sealed chain regardless. One thing a consumer must still not assume: **absence from a listing is not deletion** — it was already not proof of absence ([Hints, Not Authority](#hints-not-authority)), and a relay exercising the discovery MAY above is one concrete reason a live-looking listing is incomplete.

`key=` is the reverse lookup "which identities has this key ever been proved into." The value is matched **byte-for-byte** against the multibase public-key strings whose memberships entered the chain's **effective key state** ([PROTOCOL.md → Key Possession](https://protocol.dfos.com/spec#key-possession)) under any of the chain's **accepted operations — every operation, not just the current head**: an identity matches when its genesis key is the key, or when any accepted update ever introduced the key with a valid possession proof, whether or not a later update rotated it out. The filter is deliberately **has-ever-proved** rather than current-state — and proved rather than declared: a **void** membership (a key some chain listed without its holder's proof) never indexes, so a hostile or defective listing neither surfaces the chain in the key's recovery results nor burns the key against its true holder's own use. Its consumers are audit ("what has this key ever controlled") and key-loss recovery, and both need the full proved history: a holder rediscovering identities from a restored key may hold exactly the keys that later updates rotated out, so a current-state match would hide the chains that matter most — while current state is always one fetch away, per [Hints, Not Authority](#hints-not-authority). One key may match many identities, and a deleted identity **always** still matches — `key=` is a resolution shape, so the discovery-vs-resolution rule above requires the row rather than permitting it. The row's `isDeleted` field records the deletion, and a consumer that does not want deleted rows filters on that field client-side (this route defines no `isDeleted` parameter). There is no key-class or role column: which array carries the key, and whether it is current, is the chain's answer, not the index's. Like `signer=` on the content family, this is an **actor-axis verification outcome**, proof-tier — a proved key is the material through which an actor acts, the match is computed from accepted, signature-verified operations and their embedded proofs alone (no held-bytes dependence), and it reveals nothing not already derivable from the public per-chain log. The value is matched as an opaque string: a string no operation ever proved simply matches nothing — no format validation, no `400`.
...
When `titleContains` is present, the query is implicitly restricted server-side to `publicRead=true` rows. A non-public chain's title is never projected, and `titleContains` MUST NOT be usable to probe non-public rows. Explicitly combining `titleContains` with `publicRead=false` is a **400** (reference prose: `invalid filter combination` — illustrative only, per the [contract's error body](https://protocol.dfos.com/relay-contract#error-body)).
...
Enumerates the relay's held public credentials, `cid` ascending by default; `order=createdAt.desc` / `order=ingestedAt.desc` select the shared recency ordering described above.
...
Parameters: `issuer` (optional exact DID — `400` when malformed), `resource` (optional exact match against an `att[].resource`; when the requested resource starts with `chain:`, the `chain:*` wildcard bucket is always unioned in because a `chain:*` grant may authorize the named chain), `action` (optional exact match against an `att[].action`, with the same candidate-match posture as `resource`), `order` (optional `createdAt.desc` or `ingestedAt.desc`; lexical CID order when absent), `after` (a credential-`cid` keyset cursor in lexical mode or an opaque ordered cursor in ordered mode), `limit` (default 100, max 1000). A credential's `createdAt` is its JWT numeric `iat` normalized to ISO 8601; `ingestedAt` is when this relay accepted it. Filters are ANDed. Rows project the verified public audience (`aud`, necessarily `"*"`) and unix expiry (`exp`); timestamps remain cursor material and are not fields in the wire row. Expired rows remain enumerable and clients may filter them locally.
...
Enumerates the [credit projection](#credit-projection)'s rows: who _public_ head documents say made them. This is the sanctioned person-to-public-works lookup — `did=X` answers "which publicly readable documents credit X," the query [CONTENT-MODEL.md](https://protocol.dfos.com/content-model#credits) refuses to serve for anything less than public content.
...
Every row is amber and assertion-tier: it restates the current public head document, carries no claim token, and makes no validity claim (`hasClaim` is byte-presence). A consumer that needs the proof tier fetches the chain, re-hashes the head document, and runs the [content model](https://protocol.dfos.com/content-model#verification-algorithm) verification algorithm over the embedded entry — the row's only job is to say which documents are worth fetching. Join `contentId` against [`/index/v0/content`](#content-chains-get-indexv0contentcontentidcreatordidsignerdiddocschemadocumentcidpublicreadisdeletedtitlecontainsorderaftercontentidlimitn) for titles, schemas, and clocks, and `did` against [`/index/v0/identities`](#identities-get-indexv0identitiesdidkeyhaspublicprofilenamecontainsorderafterdidlimitn) for profiles.
...
- **One basis time governs every temporal check.** This is core protocol, normative in [PROTOCOL.md → Time basis](https://protocol.dfos.com/spec#time-basis): a committed artifact, an operation in a chain and anything carried inline in it, resolves against the operation's own `createdAt`; an ephemeral presentation resolves against now. Two relays reach the same verdict on the same committed operation whenever each one ingests it, and the admission verdict never enters the replicated log. Peer-log ingestion inherits the **peer's** admission discipline, so choosing peers is a trust decision.
...
All other rejections are permanent. Permanent rejections MUST NOT be retried. In particular, an identity operation refused as a conflicting extension is a **permanent** rejection: the log this relay serves is linear, and no state that arrives later admits a second child of a committed parent into it.
