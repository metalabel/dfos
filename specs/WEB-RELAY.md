...
- **Maintenance.** The index is fully re-derivable from the operation log plus held blobs. Reference implementations maintain it incrementally from the operation log and expose a rebuild path for pre-existing corpora; either way the serving contract is identical.
