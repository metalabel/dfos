...
- **First encounter proves nothing about ownership.** Identity operations are public data — anyone can embed a copy, or a prefix, of any published chain in a document they control — so encountering a chain in an app description says nothing about who put it there. A consumer that ingests carried chains into a store that also holds identities under its own authority MUST refuse or segregate a carried chain whose derived DID it already holds: replaying public genesis operations into a fresh store slot is otherwise a chain-substitution vector, and the monotonicity discipline above begins only after a first accepted state — it cannot defend the first encounter.
