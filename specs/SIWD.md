...
The member set is closed: this table is the registry. Every member is security-relevant, and a misspelled `redirect_uris` must never silently degrade into "no allowlist at all" — so a consumer MAY treat a document carrying unrecognized members as invalid, and new members are added by amending this table, never ad hoc. A document missing a required member is not an app description at all — consumers MUST treat it as invalid, and a required member that is present but empty (a zero-length `name`, an empty `redirect_uris` array) is missing in everything but syntax: consumers MUST treat it the same way. An optional member that is present but empty — an empty `identity_chain` — is likewise malformed rather than merely absent. A document that omits every optional member remains exactly as valid as it was before any optional member existed.
