...
**The verifier MUST have minted the nonce it checks.** A nonce handed to the verifier by the party presenting the artifact — in a callback parameter, a request body, or a header — is not a replay defense at all: an attacker replaying a captured JWS supplies the nonce that JWS already contains, and the comparison passes trivially. The expectation MUST be server-side state the verifier established before the challenge was issued, or a value the verifier can independently validate without consulting the presenter (a signed, expiring token it minted itself). A verifier that accepts a nonce expectation from the presenter has implemented the check and none of the protection.
