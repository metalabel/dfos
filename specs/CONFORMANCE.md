...
- **Key-proof verification** (if it completes key-proof ceremonies; KEY-PROOF `0.x`) —
  ...
  byte-compare it against the presented payload segment, so a reordered or re-spaced
  payload signed over its own serialization rejects), audience byte-equality
